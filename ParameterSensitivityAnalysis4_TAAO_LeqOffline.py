...
S_rs, S_Lsigma, S_omegar, S_alpha = [], [], [], []
if Jadot2009:
    S_rreq, S_Leq = [], []
coordinate = []
for rind, Tload in enumerate(range_Tload):
    for cind, omega_cmd in enumerate(range_omega):
        ...
        if not Jadot2009:
            ...
            a3 = mag((im.Leq*iMs-psiMm, im.Leq*iTs))
            a4 = mag((0, psiMm))
            ...
            S_alpha.append(     a3 *(im.alpha)    /den)
            S_omegar.append(    a4 *abs(omega_cmd)/den)

            # 2. Sensivity Check by Ruan2016。这个是错误的，因为分母选为omega_e*psiMm，实际上分母是\varepsilon！而且误差向量还没归一化。
            # check out different norms: max or sum
            # a1 = mag((iMs, iTs))
            # a2 = mag((piMs-iTs*omega_e, piTs+iMs*omega_e))
            # a3 = mag((im.Leq*iMs-psiMm, im.Leq*iTs))
            ...
            # S_rs.append(        a1                    *(im.rs)     /(abs(omega_e)*psiMm))
            # S_Lsigma.append(    a2                    *im.Lsigma   /(abs(omega_e)*psiMm))
            # S_alpha.append(     a3                    *(im.alpha)  /(abs(omega_e)*psiMm))
            # S_omegar.append(    a4                    *abs(omega_cmd)/(abs(omega_e)*psiMm))
...
if Jadot2009:
    ...
    fig, axes = plt.subplots(ncols=2, nrows=2, dpi=125, sharex=True, sharey=True); # modifying dpi would change the size of fonts
    ...
    def ct(A,ax):
        ...
        CS = ax.contour(X_mesh*100, Y_mesh*100, A, 3, linewidths=0.5, colors='k')
        CS = ax.contourf(X_mesh*100, Y_mesh*100, A, 3, cmap=plt.cm.summer, alpha=0.5)
        CB = colorbar(CS, shrink=0.8, extend='both', ax=ax) # 2 ways: http://stackoverflow.com/questions/21952100/setting-the-limits-on-a-colorbar-in-matplotlib 
        ax.set_xlim(-1*100,1*100)
        ax.set_ylim(-2*100,2*100)

    for S, tit, ax in zip([S_rs, S_Lsigma, S_alpha, S_omegar], [r'$r_s$', r'$L_\sigma$', r'$\alpha$', r'$\omega_r$'], ax_list):
        A = griddata(coordinate, S, (X_mesh,Y_mesh))
        ct(A, ax)
        ax.set_title(tit)
        # ax.locator_params(nbins=4)
    ax_list[2].set_xlabel(r'$\omega_r (\%\omega_{rN})$')
    ax_list[3].set_xlabel(r'$\omega_r (\%\omega_{rN})$')
    ax_list[0].set_ylabel(r'$T_L (\%T_{LN})$')
    ax_list[2].set_ylabel(r'$T_L (\%T_{LN})$')
