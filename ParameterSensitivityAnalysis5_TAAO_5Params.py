...
if Jadot2009:
    ...
    fig, axes = plt.subplots(ncols=2, nrows=3, dpi=150, sharex=False, sharey=True); # modifying dpi would change the size of fonts
    ax_omg = subplot(3, 1, 3) # the 3rd of 3-row-1-col subplot
    fig.subplots_adjust(right=0.95, bottom=0.15, top=0.95, left=0.15, hspace=0.67, wspace=0.01)
    ax_list = axes.ravel()
    X_mesh, Y_mesh = meshgrid(range_x, range_y)
    ...
    fig_subtitle = ['a', 'b', 'c', 'd', 'e']
    index = 0
    for S, tit, ax in zip([S_rs, S_Lsigma, S_rreq, S_Leq, S_omegar], [r'$\tilde r_s$', r'$\tilde L_\sigma$', r'$\tilde r_{req}$', r'$\tilde L_{\mu}$', r'$\tilde \omega$'], ax_list):
        ...
        if S!=None:
            ...
            # ax.set_title(tit+'~$\\text{的归一化误差}$', fontsize=10, **{'fontname':'Times New Roman'})
            # ax.set_xlabel('$\\omega_r [\\%\\omega_{rN}]$\n(%s)'%(fig_subtitle[index]))
            ax.set_xlabel('(%s)'%(fig_subtitle[index]))
    ...
    savefig(r'./ParamSensitivity.eps', format='eps', dpi=300)
